...
export function withRename<T, S extends Constructor<MotionMappable<T>>>(superclass: S): S & Constructor<MotionRenameable<T>> {
  return class extends superclass implements MotionRenameable<T> {
     ...
     * For instance:
     *
     * - `transform$.rename({ path: 'translate' })` is equivalent to
     *   `transform$.map(transform => transform.translate)`
     *
     * - `transform$.rename({ path: 'translate.x' })` is equivalent to
    ...
    rename<
      ...
      R extends M[K],
      U extends {[P in R]: V}
    >(mapping: M & { mapping: string }): ObservableWithMotionOperators<U> {
