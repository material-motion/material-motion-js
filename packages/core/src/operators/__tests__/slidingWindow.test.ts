...
describe('motionObservable.slidingWindow',
  () => {
    ...
    it(`should not dispatch until the array is the correct length`,
      () => {
    ...
    it(`should remove old items as new ones are received`,
      () => {
        ...
        expect(listener).to.have.been.calledTwice.and.to.have.been.calledWith([2, 3, 4]);
      }
    );
    ...
    it(`should have a default size of 2`,
      () => {
        ...
        expect(listener).to.have.been.calledOnce.and.to.have.been.calledWith([1, 2]);
      }
    );
  }
);
