  ...
  value$ = new MotionObservable<number>(
    (observer: Observer<number>) => {
      ...
      const subscriptions: Array<Subscription> = [
        ...
        this.tension$._map(
          (tension: number) => 194 + 3.62 * (tension - 30)
        )._map(toValueWithKey('stiffness')).subscribe(updateSpringConfig),

        this.friction$._map(
          (friction: number) => 25 + 3 * (friction - 8)
        )._map(toValueWithKey('damping')).subscribe(updateSpringConfig),

        this.threshold$._map(
          toValueWithKey('restDisplacementThreshold')
        ).subscribe(updateSpringConfig),
